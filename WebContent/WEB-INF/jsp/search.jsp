...
<div class = "searchContent">
    <div class = "filter">
        ...
        <div class="itemCount">
            <span>共找到1000件商品 <b style="color: red">1</b>/100</span>
            <a href="#">
                <i class="previousPage">&lt</i>
            </a>
            <a href="#">
                <i class="nextPage">&gt</i>
            </a>
        </div>
    </div>
    <!--过滤条件结束-->
         ...
         <div class="items">
        		...
        		<c:if test="${itemList != null }">
        			<c:forEach items="${itemList }" var="item">
						<div class = "item">
			                <a href="${pageContext.request.contextPath }/item/${item.id }.html"><img src="img/${item.image }"></a>
			                <p class = "price">￥${item.price/100.0 }</p>
			                <div class = "desc">${item.title } </div>
			                <p class = "collect"><b style="color: #646fb0">${item.collect}</b>个人收藏</p>
			                <span class="selfSupport">自营</span> 
			                <span class="new">新品</span>
			            </div>
					</c:forEach>
					
					<c:if test="${fn:length(itemList) == 0}">
		        			<div style = "color:#cccccc; margin:100px 0px 500px 500px; font-size:16px;"><h3>没有对应的商品！</h3></div>
		        	</c:if>
